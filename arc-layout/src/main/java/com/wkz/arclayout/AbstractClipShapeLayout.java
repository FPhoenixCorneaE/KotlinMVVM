...
public abstract class AbstractClipShapeLayout extends FrameLayout {
    ...
    private ClipPathManager clipPathManager = new ClipPathManagerImpl();
    private boolean requiersShapeUpdate = true;
    private Bitmap clipBitmap;

    final Path rectView = new Path();

    public AbstractClipShapeLayout(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public AbstractClipShapeLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public AbstractClipShapeLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private boolean requiresBitmap() {
        return isInEditMode() || (clipPathManager != null && clipPathManager.requiresBitmap()) || drawable != null;
    }
            ...
            if (width > 0 && height > 0) {
                ...
                if (requiresBitmap()) {
                    ...
                    if (drawable != null) {
                        ...
                        canvas.drawPath(clipPath, clipPathManager.getPaint());
                    }
                }
    ...
    public ViewOutlineProvider getOutlineProvider() {
        return new ViewOutlineProvider() {
            ...
            public void getOutline(View view, Outline outline) {
                if (clipPathManager != null) {
                    final Path shadowConvexPath = clipPathManager.getShadowConvexPath();
    ...
    public void setClipPathCreator(ClipPathManagerImpl.ClipPathCreator createClipPath) {
        ((ClipPathManagerImpl) clipPathManager).setClipPathCreator(createClipPath);
        requiresShapeUpdate();
    }

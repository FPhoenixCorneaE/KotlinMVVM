package com.wkz.rxretrofit.network.ssl
...
class NoSSLv3SocketFactory : SSLSocketFactory {
    ...
    private fun makeSocketSafe(socket: Socket): Socket {
        var s = socket
        when (s) {
            is SSLSocket -> s = NoSSLv3SSLSocket(s)
        }
        return s
    }
    ...
    private inner class NoSSLv3SSLSocket internal constructor(delegate: SSLSocket) :
        DelegateSSLSocket(delegate) {

        override fun setEnabledProtocols(protocols: Array<String>) {
            //仅能设置服务器支持的加密规范
            super.setEnabledProtocols(arrayOf("SSLv3", "TLSv1", "TLSv1.1", "TLSv1.2"))
        }
    }

    open inner class DelegateSSLSocket internal constructor(private val delegate: SSLSocket) :
        SSLSocket() {
